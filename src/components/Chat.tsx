...
const toImage = async (
  ...
  try {
    ...
    console.error(e)
    handleSaveMessage(String(e), setSaving)
...
const Chat = memo(
  ({ messages, config, showTopicList, topicName }: ChatProps) => {
    ...
    const handleSaveMarkdown = useCallback(async () => {
      ...
      if (config.export.markdown.mode === UserMessageMode.TITLE) {
        void message.warning(
          '如果用户消息中含有多行文件，应在设置中设置导出形式为“引用块”'
        )
      }

      await saveMarkdown(
        config.export.markdown.mode,
        path,
        topicName,
        messages,
        setProgress
